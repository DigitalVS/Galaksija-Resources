...
FAST:
  di
  ret
Z32:                            ; CP = HL - DE, Z and C flags valid
  ld    a, h
  cp    d
  ret   nz
  ld    a, l
...
GO8C9:
  jp    Z8C9
NUMBER:
  db    28                      ; ROM A version number

  push  af                      ; Video interrupt routine
  push  bc
  push  de
  push  hl
  ld    hl, STACK + 8
...
DIM = $
  ...
  xor   a
  call  HLX4
  ld    ($2A99), hl             ; $2A99, 2 byte long, init value 00, 16 * ARR$ + 16
  rst   Z48
STRMAT:
  rst   Z807
  inc   hl
...
GOTOVO:
  ld    ($406A - RAM), hl
  ld    sp, STACK + 5
  ld    hl, $C900 + HOR         ; Hor. pos. and RET
  push  hl                      ; Video link
  dec   sp
  push  hl
  ld    a, $0C
  rst   Z16                     ; call $1000 for ROM B
NEW:
  call  BEC4
  ld    de, BASIC
  add   hl, de
...
ZAF2:
  ...
  jr    nz, ZB09
  ld    hl, STACK + 8
  call  WAIT                    ; Until scroll ends
  push  hl
  inc   hl
  inc   (hl)
  call  WAIT                    ; For one whole screen
  or    a
...
IZLAZ2:
  ...
  jr    c, IZLAZ
  ld    hl, TAB3
  ld    bc, $45B                ; 4BY table + arrow left
NFND:
  cp    (hl)
  jr    z, FNDSL
  inc   hl
